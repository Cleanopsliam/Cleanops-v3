...
export default function DaysSelect({ name, defaultValue = [], label = 'Available days' }: Props) {
  ...
  // Keep UI in sync if defaultValue changes (e.g., editing)
  useEffect(() => {
    setValue(defaultValue ?? [])
  }, [defaultValue?.join(',')]) // join for a stable dep

  // ---- Reset behaviour ----
  // A) reset when the parent <form> is reset (native reset)
  useEffect(() => {
  ...
  // B) reset after server action completes (custom event)
  useEffect(() => {
    const onCustomReset = () => setValue([])
    window.addEventListener('staff-form-reset', onCustomReset)
    return () => window.removeEventListener('staff-form-reset', onCustomReset)
  }, [])
  ...
  // Summary text when there are too many chips to render inline
  ...
  // Position (desktop). On mobile we do fullscreen sheet.
      ...
      >
        <div className="min-h-[1.5rem] flex flex-wrap gap-1">
          {(chips.length > 0 && chips.length <= 3)
            ? chips.map(lbl => (
                <span key={lbl} className="badge">{lbl}</span>
              ))
    ...
    <div className="grid grid-cols-7 gap-2 sm:grid-cols-7">
      {DAYS.map(d => {
        ...
        return (
          <label
            key={d.id}
            className={`day-tile ${active ? 'day-on' : ''}`}
            aria-pressed={active}
          >
            <input
              type="checkbox"
